...
function ConsultaServidor(url) {
    ...
    $.post(url, function (data, status) {
        ...
        let lineanegocios = JSON.parse(data);
        let total_lineanegocios = lineanegocios.length;
        ...
        for (var i = 0; i < lineanegocios.length; i++) {
            ...
            tr += '<tr>' +
                '<td>' + (i + 1) + '</td>' +
                '<td>' + lineanegocios[i].Codigo.toUpperCase() + '</td>' +
                '<td>' + lineanegocios[i].Descripcion.toUpperCase() + '</td>' +
                '<td><button class="btn btn-primary fa fa-pencil btn-xs" onclick="ObtenerDatosxID(' + lineanegocios[i].IdLineaNegocio + ')"></button>' +
                '<button class="btn btn-danger btn-xs  fa fa-trash" onclick="eliminar(' + lineanegocios[i].IdLineaNegocio + ')"></button></td >' +
                '</tr>';
        }

        $("#tbody_LineaNegocios").html(tr);
        $("#spnTotalRegistros").html(total_lineanegocios);
